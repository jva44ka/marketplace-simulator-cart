...
1. **Добавление/удаление товара** — работает только с корзиной в БД, без обращения к складу.
...
4. **Истечение резервирования** — сервис товаров публикует событие в топик `reservation-expired-topic`. Kafka-консьюмер обрабатывает событие (в текущем флоу корзина уже очищена к этому моменту, поэтому является страховочным механизмом).
